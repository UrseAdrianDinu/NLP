...
if __name__ == '__main__':
    ...
    new_tweets_csv = "new_tweets.csv"
    new_tweets_df = pd.read_csv(new_tweets_csv)
    new_tweets_df = preprocess_data(new_tweets_df)
    new_tweet_vectors, _ = vectorize_text(new_tweets_df['cleaned_text'], vectorizer)

    if best_model is not None:
        new_predictions = best_model.predict(new_tweet_vectors)
        new_tweets_df['predicted_label'] = new_predictions
        print("Predictions on new dataset:")
        print(new_tweets_df[['text', 'predicted_label']])
    else:
